...
const connect = (handler) => {
  ...
  let port = new SerialPort({ path: '/dev/tty.usbmodem1051DB2D6CE42', baudRate: 9600 }); // for mac testing, CHANGE FOR WINDOWS
  ...
  //let port2 = new SerialPort({ path: "COM15", baudRate: 9600 });//melodie's port (check in device manager for port) always 15. both are 15 for some reason
